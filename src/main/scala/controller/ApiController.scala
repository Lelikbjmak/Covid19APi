...
class ApiController(covidDetailsService: CovidDetailsService, apiControllerRouteValidator: ApiControllerRouteValidator, countryRetrieveService: CountryRetrieveService) extends DefaultJsonProtocol {
  ...
  private val route: Route = pathPrefix("covid" / "countries") {
    respondWithHeaders(
      ...
      pathEnd {
        get {
          val countries = countryRetrieveService.fetchAllCountries()
