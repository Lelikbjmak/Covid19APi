...
class ApiController(covidDetailsService: CovidDetailsService, apiControllerRouteValidator: ApiControllerRouteValidator) extends JsonSupport {
          ...
          entity(as[List[String]]) { countryList => {
            validate(apiControllerRouteValidator.validateRequestBody(countryList),
              ...
              validate(apiControllerRouteValidator.validateRequestParamAndPathVariable(dayString),
                ...
                val covidDetailsForParticularDay: Future[ResponseDto[CovidMaxMinCasesDto]] = covidDetailsService.getMaxAndMinCovidCasesForCountryListForDay(countryList, requestedDay)
            ...
            entity(as[List[String]]) { countryList => {
              validate(apiControllerRouteValidator.validateRequestBody(countryList),
                ...
                validate(apiControllerRouteValidator.validateRequestParamAndPathVariable(from, to),
                  ...
                  val covidDetailsForParticularDay: Future[ResponseDto[CovidMaxMinCasesDto]] = covidDetailsService.getMaxAndMinCovidCasesForCountryListForTerm(countryList, firstDay, lastDay)
